texts = {
    "uk": {
        "select_language": "Привіт! Якою мовою тобі зручніше спілкуватися? 🌐",
        "language_selected": "Обрано мову: 🇺🇦 Українська",
        "select_service": "Що тебе цікавить? 🏡",
        "rent": "🔍 Добре! Зараз підберемо оренду.",
        "buy": "💰 Чудово! Зараз знайдемо для тебе житло для купівлі.",
        "sell": "📦 Чудово! Давай допоможемо здати або продати твоє житло.",
        "select_rooms": "Скільки повинно бути кімнат у квартирі?",
        "rooms_done": "Обрано: {rooms}",
        "no_rooms": "нічого 😢",
        "select_districts": "У яких районах шукаєш квартиру?",
        "districts_done": "Обрані райони: {districts}",
        "no_districts": "нічого 😢",
        "select_budget": "На яку суму ти розраховуєш? 💸",
        "budget_done": "Обрано діапазони: {budget}",
        "no_budget": "нічого 😢"
    },
    "en": {
        "select_language": "Hello! What language do you prefer to use? 🌐",
        "language_selected": "Selected language: 🇬🇧 English",
        "select_service": "What are you interested in? 🏡",
        "rent": "🔍 Great! Let's find a rental for you.",
        "buy": "💰 Excellent! We'll find a property to buy.",
        "sell": "📦 Let's help you rent or sell your property.",
        "select_rooms": "How many rooms should the apartment have?",
        "rooms_done": "Selected: {rooms}",
        "no_rooms": "nothing 😢",
        "select_districts": "Which districts are you interested in?",
        "districts_done": "Selected districts: {districts}",
        "no_districts": "nothing 😢",
        "select_budget": "What is your budget range? 💸",
        "budget_done": "Selected ranges: {budget}",
        "no_budget": "nothing 😢"
    }
}
